...
def check_dup_points(points_str):
	temp = point_parser(points_str)
	if (len(temp) == 2):
		if (temp[0] == temp[1]):
			return True
	return False 
...
class Street_Database:
	...
	def change(self, user_input):
		...
		if (check_dup_points(user_input[2])):
			print ("Error: Cannot have a street that is made up of the same 2 points",file=sys.stderr)
			return
	...
	def inter_point(self,street,edge_list):
		...
		if (len(self.street_database) == 0):
			...
			for i in range(0, len(current_segments)-1):
				...
				if (len(final_ip_list) >=2):
					for k in range(0,len(final_ip_list)-1):
						edge_list.append((final_ip_list[k],final_ip_list[k+1]))
	...
	def graph(self):
		...
		if (len(self.street_database) == 0):
			...
			if (len(final_edge_list) == 1):
				...
				print ("E {<",final_edge_list[0][0],",",final_edge_list[0][1],">}",sep = "")
				return
			...
			if(len(final_edge_list) > 1):
				...
				for i in range(0,len(final_edge_list)-1):
					print ("<",final_edge_list[i][0],",",final_edge_list[i][1],">,",sep = "", end='')

				print ("<",final_edge_list[i+1][0],",",final_edge_list[i+1][1],">}\n",sep = "", end='')
				return
